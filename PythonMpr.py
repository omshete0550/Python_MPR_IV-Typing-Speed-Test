...
class MainWindow:
  ...
  def __init__(self, root):
    ...
    self.label_text = Label(root,
                            text="Welecom to typeing speed calculator",
                            wraplength=500)
  ...
  def stop(self):
    ...
    self.accuracy = round(
      difflib.SequenceMatcher(None, self.label_text.cget("text"),
                              self.user_text.get("1.0", 'end-1c')).ratio() *
      100)
  ...
  def new_text(self):
    self.label_text.config(
      text=self.text[random.randint(0,
                                    len(self.text) - 1)])
